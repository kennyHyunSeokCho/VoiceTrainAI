...
def fetch(
    filename,
    source,
    savedir="/tmp/pretrained_model_checkpoints",
    overwrite=False,
    save_filename=None,
    ...
    # 수정: filename이 절대경로면 source를 붙이지 않음
...
def from_hparams(
    ...
    **kwargs,
    ...
    if savedir is None:
        ...
        savedir = f"/tmp/pretrained_models/{clsname}-{hashlib.md5(source.encode('UTF-8', errors='replace')).hexdigest()}"
...
def from_scripted(filename, source, savedir=None):
    ...
    if savedir is None:
        savedir = f"/tmp/pretrained_models/{filename}-{hashlib.md5(source.encode('UTF-8', errors='replace')).hexdigest()}"
